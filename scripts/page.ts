function renderFileStatus(handle: FileSystemFileHandle | null, isBusy: boolean, isSaved: boolean) {
...
function renderLists(itemsOfTypes: { [type: string]: string[] }, selType: string, selItem: string, cb: (type: string, item: string) => void) {
  const sidebar = document.getElementById("sidebar") as HTMLDivElement;
  const list = document.getElementById("list") as HTMLDivElement;
  const scrollTop = sidebar.scrollTop;

  list.innerHTML = "";
  list.append(...createLists(itemsOfTypes, selType, selItem, cb));
  sidebar.scrollTop = scrollTop;
}

function renderItemMenu(types: string[], cb: (type: string) => void) {
...
function createLists(itemsOfTypes: { [type: string]: string[] }, selType: string, selItem: string, cb: (type: string, item: string) => void): HTMLElement[] {
  let elements: HTMLElement[] = [];
  
  for (let type in itemsOfTypes) {
    elements.push(...createList(type, itemsOfTypes[type], selType, selItem, cb));
  }

  return elements;
}

function createList(type: string, items: string[], selType: string, selItem: string, cb: (type: string, item: string) => void): [HTMLDivElement, HTMLDivElement] {
  ...
  for (let item of items) {
    outer.append(createListItem(type, item, selType != "" && selType == type && selItem == item, cb));
  }
